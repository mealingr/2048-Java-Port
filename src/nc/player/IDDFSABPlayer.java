...
public final class IDDFSABPlayer extends AbstractPlayer
{
    ...
    public IDDFSABPlayer(final Random random,
                         final Game game)
    {
        super(random);
        this.game = game;
    ...
    public static final boolean isTileInGrid(final int[][] grid,
                                             final int row,
                                             final int column)
    {
        return row > -1 &&
    ...
    public static final boolean isTileEmpty(final int[][] grid,
                                            final int row,
                                            final int column)
    {
    ...
    public static final int[] findFarthestTile(final int[][] grid,
                                               int row,
                                               int column,
                                               final int[] vector)
    {
        ...
        while (isTileInGrid(grid, row, column) &&
               isTileEmpty(grid, row, column));
    ...
    public static final double getMonotonicity(final int[][] grid)
    {
        ...
        for (int row = 0; row < grid.length; row++)
        {
            ...
            while (nextColumn < grid[row].length)
            {
                ...
                final int currentPower = !isTileEmpty(grid, row, currentColumn) ? (int) (Math.log(grid[row][currentColumn]) / Math.log(2)) : 0;
                final int nextPower = !isTileEmpty(grid, row, nextColumn) ? (int) (Math.log(grid[row][nextColumn]) / Math.log(2)) : 0;
        ...
        for (int column = 0; column < grid[0].length; column++)
        {
            ...
            while (nextRow < grid.length)
            {
                ...
                final int currentPower = !isTileEmpty(grid, currentRow, column) ? (int) (Math.log(grid[currentRow][column]) / Math.log(2)) : 0;
                final int nextPower = !isTileEmpty(grid, nextRow, column) ? (int) (Math.log(grid[nextRow][column]) / Math.log(2)) : 0;
                if (currentPower > nextPower)
                {
    ...
    {
        return getSmoothness(grid) * smoothnessWeight +
               getMonotonicity(grid) * monotonicityWeight +
               Math.log(countEmptyTiles(grid)) * emptyWeight +
               getMaxValue(grid) * maxWeight;
    }
    ...
    public static final void mark(final boolean[][] marked,
                                  final int[][] grid,
                                  final int row,
                                  final int column,
                                  final int value)
    {
        if (isTileInGrid(grid, row, column) &&
            !isTileEmpty(grid, row, column) &&
    ...
    public static final double[] search(final int[][] grid,
                                        final int depth,
                                        final double alpha,
                                        final double beta,
                                        int positions,
            ...
            for (int direction = Game.UP; direction <= Game.LEFT; direction++)
            {
                ...
                if (!Arrays.deepEquals(clone, grid))
                {
                    ...
                    else
                    {
                        ...
                        {
                            result = search(clone, depth - 1, bestScore, beta, positions, cutoffs, !playerTurn, winningTile);
    ...
    public static final double[] iterativeDeep(final int[][] grid,
                                               final double alpha,
                                               final double beta,
                                               final int winningTile,
                                               final long maxSearchTime)
    {
